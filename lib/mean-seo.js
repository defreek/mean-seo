...
exports.init = function(options) {
    ...
    return function SEO(req, res, next) {
        ...
        if (useragent && (useragent.toLowerCase().indexOf('facebookexternalhit') !== -1 || useragent.toLowerCase().indexOf('facebot') !== -1)) isKnowBot = true;
        ...
        if (escapedFragment !== undefined || isKnowBot) {
            ...
            cache.get(key, function(err, page) {
                if (err) {
                    ...
                    browser.crawl(key.replace('1roof.be', 'apollo2.herokuapp.com'), function(err, html) {
                        if (err) {
                            next(err);
                        } else {
                            //Save page to cache 
